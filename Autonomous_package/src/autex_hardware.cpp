...
public:
    IrcNode() : Node("ircNode"), state_(State::MOVING_FORWARD), count_(0)
    {
        ...
        subscription_2 = this->create_subscription<sensor_msgs::msg::Imu>("/imu", 10, std::bind(&IrcNode::thetaNode, this, _1));
...
private:
    ...
    void thetaNode(const sensor_msgs::msg::Imu &msg)
    {
        theta = quatToEuler(msg.orientation);
    }
    ...
    void timer_callback()
    {
        ...
        vel.linear.x = (is_moving && linear_velocity_ >= 0.075) ? ((linear_velocity_ > 0.5) ? 0.5 : linear_velocity_) : 0.0;
        vel.linear.x = 2*vel.linear.x;
        ...
        if (direction == 1 || direction == -1)
        {
            ...
            else
            {
                ...
                vel.angular.z = 2*(abs(vel.angular.z) > 0.5 ? 0.5 * abs(vel.angular.z) / vel.angular.z : vel.angular.z);
            }
        }
        else
        {
